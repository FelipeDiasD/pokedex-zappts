...
export default function Home() {
  ...
  useEffect(() => {
    async function loadData() {
      const pokemon = await api.get('pokemon?limit=300&offset=0') //limitado a 150(região de kanto) mas pode ser aumentado
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.middleSection}>
        <Text style={styles.greetingsAskToChoose}>
          Witch pokémon you are looking for?
        </Text>

        <TouchableOpacity
          onPress={() =>
            navigation.navigate('Search', { pokemonData: returnedPokemons })
          }
        >
      ...
      <FlatList
        ...
        renderItem={({ item, keyExtractor }) => (
          <PokemonCardItem pokemon={item} id={keyExtractor} />
        )}
      />
    </SafeAreaView>
  )
