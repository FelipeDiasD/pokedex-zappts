...
export default function PokemonCardItem (pokemon, id) {
    ...
    const [pokemonTypes, setPokemonTypes] = useState ([])
     ...
     useEffect ( () => {
         ...
         const retunedTypes = await selectedPokemon?.data?.types?.map(tipos => tipos?.type?.name)
  ...
  const imageEndpoint = pokemonDetails?.data?.sprites?.other["official-artwork"].front_default
  ...
  const mainMove = pokemonDetails?.data?.moves[4]?.move
  ...
  const navigation = useNavigation()
     function handleNavigation () {
          navigation.navigate("Details",{pokemonName: pokemon?.pokemon?.name, 
    ...
    return (
        ...
        <TouchableOpacity 
         ...
         <View style = {styles.cardItems}>
              <Text 
                 style = {styles.cardText}>{`${pokemon?.pokemon?.name}`}
              </Text>


             <PokemonTypeCard text = {`${pokemonTypes[0]}`}/>

             {pokemonTypes.length > 1 &&
             <PokemonTypeCard text = {`${pokemonTypes[1]}`}/>
             }
         </View>
...
const styleForContainer = (typeColor) => StyleSheet.create({
    container: {
